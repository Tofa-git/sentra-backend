...
const msRoomGradeModel = db.masterRoomGrade;
const AppError = require('../../utils/appError')


const addMsRoomGrades = async (req, res) => {
    ...
    if (req.body && Array.isArray(req.body)) {
        ...
        await msRoomGradeModel.bulkCreate(datas).then(data => {
...
const getMsRoomGrades = async (req, res, next) => {
    ...
    const data = await msRoomGradeModel.findAll({
        data: [
            'id',
            'sequence',
            'description',            
...
const getMsRoomGrade = async (req, res) => {
    const id = req.params.id;
    const data = await msRoomGradeModel.findOne({
        attributes: [
            'id',
            'sequence',
            'description',            
...
const editMsRoomGrade = async (req, res) => {
    const id = req.params.id;
    // Extract userId from JWT token
    const userId = req.user.id;
    if (req.body && Array.isArray(req.body)) {
        ...
        const updatedData = await msRoomGradeModel.update(currencies, { where: { id: id } });
...
const deleteMsRoomGrade = async (req, res) => {
    const id = req.params.id;
    const deletedData = await msRoomGradeModel.destroy({ where: { id: id } });
