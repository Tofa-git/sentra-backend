...
const msCxlPolicyDetailModel = db.cxlPolicy;
const AppError = require('../../utils/appError')


const addMsCxlPolicyDetails = async (req, res) => {
    ...
    if (req.body && Array.isArray(req.body)) {
        const datas = req.body.map(
            response => {
                return {
                    ...
                    cutOffDay: response.cutOffDay,
                    time: response.description,
        ...
        await msCxlPolicyDetailModel.bulkCreate(datas).then(data => {
...
const getMsCxlPolicyDetails = async (req, res, next) => {
    ...
    const data = await msCxlPolicyDetailModel.findAll({
        data: [
            'id',
            'cxlPolicyId',
            'description',
...
const getMsCxlPolicyDetail = async (req, res) => {
    const id = req.params.id;
    const data = await msCxlPolicyDetailModel.findOne({
        attributes: [
            'id',
            'cxlPolicyId',
            'description',
...
const editMsCxlPolicyDetail = async (req, res) => {
    const id = req.params.id;
    // Extract userId from JWT token
    const userId = req.user.id;
    if (req.body && Array.isArray(req.body)) {
        ...
        const updatedData = await msCxlPolicyDetailModel.update(currencies, { where: { id: id } });
...
const deleteMsCxlPolicyDetail = async (req, res) => {
    const id = req.params.id;
    const deletedData = await msCxlPolicyDetailModel.destroy({ where: { id: id } });
...
module.exports = {
    addMsCxlPolicyDetails,
    getMsCxlPolicyDetails,
    getMsCxlPolicyDetail,
    editMsCxlPolicyDetail,
    deleteMsCxlPolicyDetail
}